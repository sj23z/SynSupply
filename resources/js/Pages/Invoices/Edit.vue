...
const props = defineProps({ customers: Array, products: Array, reps: Array, invoice: Object });

const form = useForm({
    customer_id: props.invoice.customer_id,
    sales_rep_id: props.invoice.sales_rep_id ?? '',
    invoice_date: props.invoice.invoice_date,
    language: props.invoice.language,
    invoice_discount_type: props.invoice.invoice_discount_type ?? '',
    invoice_discount_value: props.invoice.invoice_discount_value ?? 0,
    notes: props.invoice.notes ?? '',
    items: props.invoice.items.length
        ? props.invoice.items.map((i) => ({ product_id: i.product_id, description: i.description, qty: i.qty, unit_price: i.unit_price, discount_type: i.discount_type ?? '', discount_value: i.discount_value }))
        : [{ product_id: '', description: '', qty: 1, unit_price: 0, discount_type: '', discount_value: 0 }],
});
...
<template>
    <AppLayout>
        <h1 class="mb-4 text-base font-bold text-ink">تعديل الفاتورة (مسودة)</h1>
        ...
        <form @submit.prevent="submit">
            ...
            <div class="utu-card mb-4 overflow-hidden !p-0">
                ...
                <div class="p-3"><button type="button" class="utu-btn-ghost w-full !text-[12px] md:w-auto" @click="addLine">+ إضافة سطر</button></div>
            </div>
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div class="utu-card">
                    ...
                    <div class="mt-3 flex gap-2">
                        <button type="submit" class="utu-btn-dark flex-1" :disabled="form.processing">حفظ التعديلات</button>
                        <a :href="route('invoices.show', invoice.id)" class="utu-btn-ghost">إلغاء</a>
                    </div>
                </div>
            </div>
        </form>
