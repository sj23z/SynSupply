...
const props = defineProps({ rep: Object, customers: Array, totals: Object, from: String, to: String });
const from = ref(props.from);
const to = ref(props.to);
function run() { router.get(route('reports.sales-representatives.detail', props.rep.id), { from: from.value, to: to.value }); }
...
<template>
    <AppLayout>
        <div class="mb-4 flex flex-wrap items-center justify-between gap-2">
            <h1 class="text-base font-bold text-ink">تقرير مندوب: {{ props.rep.name }}</h1>
            <a :href="route('reports.sales-representatives.detail.pdf', props.rep.id) + '?from=' + from + '&to=' + to" target="_blank" class="utu-btn-ghost">طباعة / PDF</a>
        </div>
        ...
        <div class="utu-card mb-4 flex flex-wrap items-end gap-2">
            ...
            <a :href="route('reports.sales-representatives')" class="utu-btn-ghost">رجوع للتقرير</a>
