...
function run() { router.get(route('reports.sales-representatives'), { from: from.value, to: to.value }); }
function formatIqd(v) { return new Intl.NumberFormat('en-US').format(v) + ' د.ع'; }
</script>
<template>
    <AppLayout>
        <h1 class="mb-4 text-base font-bold text-ink">تقرير المندوبين</h1>
        ...
        <div class="utu-card overflow-hidden !p-0">
            ...
            <table class="w-full text-[13px]">
                <thead class="bg-grey-1 text-[12px] text-grey-text">
                    <tr><th class="px-3 py-2 text-start">المندوب</th><th class="px-3 py-2 text-start">عدد العملاء</th><th class="px-3 py-2 text-start">إجمالي المبيعات</th><th class="px-3 py-2 text-start">المردودات</th><th class="px-3 py-2 text-start">صافي المبيعات</th><th class="px-3 py-2 text-start">التحصيلات</th><th class="px-3 py-2"></th></tr>
                </thead>
                <tbody>
                    <tr v-for="r in rows" :key="r.id" class="border-t border-grey-2">
                        ...
                        <td class="px-3 py-2">{{ formatIqd(r.payments_collected) }}</td>
                        <td class="px-3 py-2 text-end"><a :href="route('reports.sales-representatives.detail', r.id)" class="utu-btn-ghost !px-2 !py-1 !text-[11px]">تفاصيل</a></td>
                    </tr>
                </tbody>
            </table>
        </div>
